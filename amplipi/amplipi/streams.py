...
class Bluetooth(BaseStream):
  ...
  def connect(self, src):
    ...
    btpon_proc = subprocess.Popen(args=btpon_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
    btdon_args = f'bluetoothctl discoverable on'
    print(f'running: {btdon_args}')
    bton_proc = subprocess.Popen(args=btdon_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
    ...
    self.proc = subprocess.Popen(args=baplay_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
    ...
    self.proc2 = subprocess.Popen(args=btmeta_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
  ...
  def disconnect(self):
    ...
    btdoff_proc = subprocess.Popen(args=btdoff_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
    btpoff_args = f'bluetoothctl power off'
    btpoff_proc = subprocess.Popen(args=btpoff_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
  ...
  def send_cmd(self, cmd):
    try:
      if cmd in self.supported_cmds and self.src != None:
        ...
        btcmd_proc = subprocess.Popen(args=btcmd_args.split(), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setpgrp)
      else:
        raise NotImplementedError(f'"{cmd}" is either incorrect or not currently supported')
    except Exception:
      pass
